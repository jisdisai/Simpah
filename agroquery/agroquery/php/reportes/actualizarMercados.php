<?php
  ...
  $con=new Conexion();
  $salida="";
  if(isset($_POST["tipoProducto"]) && ((strcmp($_POST["tipoProducto"], "")!=0))){
    ...
    if(strcmp($tipoProducto, "default")==0){
      $sql=str_replace("WHERE (tbl_producto.tbl_tipo_producto_id=".$tipoProducto.")", "", $sql);
    }

    $resultadoSql=$con->ejecutarSql($sql);
    if($con->cantidadRegistros($resultadoSql)>0){
      $salida=$salida."$(\"#select_mercado\").attr('disabled',false);";
