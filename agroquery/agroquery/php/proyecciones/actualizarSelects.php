	...
	if($con->cantidadRegistros($resultadoSql)>0){
		for($i=0; $i<$con->cantidadRegistros($resultadoSql);$i++){
			...
			if(strcmp($registro['name_tipo_producto'], "GBMN")!=0){
				$salida=$salida."$('#select_tipoProducto').prepend(\"<OPTION value='".$registro['id']."'>".$registro['name_tipo_producto']."</OPTION>\");";
			}
		...
		if($con->cantidadRegistros($resultadoSql)>0){
			for($i=0; $i<$con->cantidadRegistros($resultadoSql);$i++){
				$registro=$con->obtenerFila($resultadoSql);
				$salida=$salida."$('#select_mercado').prepend(\"<OPTION value='".$registro['id']."' >".$registro['nombre_mercado']."</OPTION>\");";
			}
